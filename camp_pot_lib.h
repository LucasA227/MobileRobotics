...
float controlePosicao(float P[], float Px, float Py, float Obst[][2], int nObs, float e0, float krho, float kRep, float limitV, float limitW, float dt, float *vRefE, float *vRefD, float eixo)
{
  ...
  float FRep[1000][2];  //Vetor utilizado para armazenar a força repulsivas das direções x e y (FRep[i][0] e FRep[i][0] respectivamente para cada i obstáculo)
  ...
  for (int i = 0; i < nObs; i++)
  {
    ...
    if(distObs <= e0) //Cálcula as forças repulsivas se estiver próximo do obstáculo
    {
...
void controleVelocidade(float VelLeft, float VRefLeft, float VelRight, float VRefRight, float ki, float kp, float *erro, int *MTL, int *MTR)
{
  ...
  *MTL = limitaMT(*MTL + int(kp*erroL + ki*erro[0])); //Controle PI da roda esquerda
  *MTR = limitaMT(*MTR + int(kp*erroR + ki*erro[1])); //Controle PI da roda direita
